...
const HeaderFooter = {
  ...
  loadHeader: async () => {
    // fetch() のパスは、そのJavaScriptを読み込んでいるHTMLファイルから参照しているため、相対パスで読み込める
  ...
  setBackgroundColors: () => {
    ...
    if (header) {
      if (isIndexPage) {
        // header.style.backgroundImageという記述は、<header style="background-image: url(assets/img/...);">と同じで、このファイルではなくHTMLファイルから参照しているため、相対パスで読み込める
