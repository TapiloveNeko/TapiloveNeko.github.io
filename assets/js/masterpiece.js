...
const MasterpieceGallery = {
  ...
  generateArtworkHTML: (artwork, index) => {
    ...
    artworkTable.innerHTML = `
      <div id="${imageID}" class="img-wrap">
        <img src="${artwork.imageSrc}" class="c-img">
      </div>
      <dl class="c-description-list">
        <dt class="c-description-term">作品名</dt>
        <dd class="c-description-definition">${artwork.name}</dd>

        <dt class="c-description-term">絵画技法</dt>
        <dd class="c-description-definition">${artwork.technique}</dd>

        <dt class="c-description-term">制作年</dt>
        <dd class="c-description-definition">${artwork.year}</dd>
      </dl>
    `;
    artworkTable
      .querySelector(`#${imageID}`)
      .addEventListener("click", () => MasterpieceGallery.openModal(artwork.imageSrc));
  ...
  changeImage: (direction) => {
    ...
    const newImageSrc = MasterpieceGallery.artworks[MasterpieceGallery.currentIndex].imageSrc;
    const modalImg = document.querySelector(".inside-modal img");
    if (modalImg) {
      modalImg.setAttribute("src", newImageSrc);
    }
